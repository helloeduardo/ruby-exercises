...
class RejectPatternTest < Minitest::Test
  ...
  def test_remove_words_ending_in_ing
    ...
    selected = []
    words.each do |word|
      selected << word unless word.slice(-3,3) == "ing"
    end
  ...
  def test_remove_words_containing_e
    ...
    selected = []
    words.each do |word|
      selected << word unless word.count("e") > 0
    end
  ...
  def test_remove_numbers
    ...
    not_numbers = []
    elements.each do |i|
      not_numbers << i unless i.class != String
    end
  ...
  def test_remove_floats
    ...
    not_numbers = []
    elements.each do |i|
      not_numbers << i unless i.class == Float
    end
  ...
  def test_remove_arrays
    ...
    remaining = []
    elements.each do |i|
      remaining << i unless i.class == Array
    end
  ...
  def test_remove_hashes
    ...
    remaining = []
    elements.each do |i|
      remaining << i unless i.class == Hash
    end
