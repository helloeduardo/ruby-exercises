...
class NestedTest < MiniTest::Test
  ...
  def test_list_of_employees_across_all_restaurants
    ...
    #=======================
    employee_names = []
    stores.each do |restaurant_name, restaurant_qualities|
      employee_names.concat(restaurant_qualities[:employees])
    end
    #or avoid concat and flatten after iterating
    #=======================
  ...
  def test_list_of_all_ingredients_across_all_restaurants
    # skip
    #=======================
    ingredients = stores.map do |store, store_info|
      stores[store][:dishes].map do |dish|
        dish[:ingredients]
      end
    end
    ingredients.flatten!
  ...
  def test_full_menu_for_olive_garden
    ...
    stores[:olive_garden][:dishes].each do |dish|
      #assign a new key in blank array list with the key = dish name and the value = dish details array
  ...
  def test_menu_accross_all_restaurants
    ...
    #=======================
    full_menu = {}
    stores.each do |restaurant, data|
